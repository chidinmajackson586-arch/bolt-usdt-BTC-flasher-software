        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex space-x-2">
            <Select defaultValue="all">
              <SelectTrigger className="bg-secondary border-gray-600 w-auto">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
        ...
        <div className="space-y-3">
          {isLoading ? (
            ...
            transactions.map((tx: any) => (
              <div key={tx.id} className="transaction-card bg-secondary rounded-lg p-4 border border-gray-700">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className={`w-12 h-12 ${getStatusColor(tx.status)} bg-opacity-20 rounded-full flex items-center justify-center`}>
                      <i className={`${getTransactionIcon(tx.token)} ${getStatusColor(tx.status).split(' ')[1]}`}></i>
                    </div>
                    <div>
                      <p className="font-semibold">{tx.token} Transfer</p>
                      <p className="text-sm text-muted-foreground">{tx.network} Network</p>
